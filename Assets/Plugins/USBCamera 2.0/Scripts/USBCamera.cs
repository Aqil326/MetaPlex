...
//#endif
        ...
        public void OnAutoWhiteBalanceTemperatureChange(bool input)
        {
            if (isNotAndroid)
            {
                CameraDebug.Log("Auto White Balance adjustment is only supported on Android");
                return;
            }
            plugin.androidJavaObject.Call<int>("setCameraDetailsParameter", deviceID, 2, input ? 1 : 0, false);
        }
        public void OnAutoExposureChange(bool input)
        {
            if (isNotAndroid)
            {
                CameraDebug.Log("Auto Exposure adjustment is only supported on Android");
                return;
            }
            plugin.androidJavaObject.Call<int>("setCameraDetailsParameter", deviceID, 3, input ? 0 : 1, false);
        }
        public void OnAutoFocusChange(bool input)
        {
            if (isNotAndroid)
            {
                CameraDebug.Log("Auto Focus adjustment is only supported on Android");
                return;
            }
            plugin.androidJavaObject.Call<int>("setCameraDetailsParameter", deviceID, 4, input ? 1 : 0, false);
        }
